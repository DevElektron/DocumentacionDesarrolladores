...
Cuando en la ruta de `/app/ventas/pedidos` se carga los pedidos al usuario el cual tiene el rol asignado de `Gerente`, se visualizará el botón `Aumentar días para Desasignación del Pedido` (Prórroga del pedido). Cuando el usuario selecciona dando clic a un pedido, se realiza una serie de validaciones para verificar si el pedido y la configuración del usuario es correcta para usar la funcionalidad de prórroga:
