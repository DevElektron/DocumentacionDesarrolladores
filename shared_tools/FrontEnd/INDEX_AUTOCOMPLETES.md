...
Este documento funciona como índice técnico de los componentes autocomplete implementados en AdministraWeb. El objetivo principal es centralizar la documentación de estos componentes para promover su reutilización, estandarizar su implementación y evitar la creación redundante de nuevos autocompletes.
  
Para cada componente se detalla:

1. Nombre.
2. Ubicación dentro del proyecto.
3. Estructura de datos devuelta (campos).
4. Parámetros utilizados para realizar búsquedas.
...
- 🧾 **Contenido:**
        ...
        `VEN:FcBaja`,  
        `VEN:Tproyecto`
- 📏 **Filtro de Búsqueda.** Hay 2 opciones:
    1. `VEN:FcBaja = 0`
    2. **Modo Gerente**: Si se le parametriza `true`, devuelve todos los vendedores cuyo Gerente es el NVEN (@NVEN_GTE) del usuario que inició sesión, mediante los almacenes a cargo de dicho usuario.
        - `AND ven.NALM in (
                SELECT
                    NALM
                FROM
...
- 📏 **Filtro de Búsqueda.** Hay dos opciones de acuerdo al valor enviado de NVEN:
    1. Si se envio NVEN con valor:
        - **Joins:**
            - `GTE:NCZO = CZO:NCZO`  
        - **Where:**
            - **Número de Vendedor asociado al Gerente**
                - `GTE:NVEN = [NVen]`
    2. Si NVEN se envió NULO (null), no se aplica filtro.
...
> 👤 **Eduardo Navarro, Sergio Tostado, Daniel Salazar**  
> 🏷️ **Versión:** 3  
